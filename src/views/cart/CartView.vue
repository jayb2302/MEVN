<template>
  <div class="p-4">
    ...
    <div v-if="cart.length > 0"> <!-- If cart is not empty -->
      <div class="flex items-center mb-4 border-b pb-4" v-for="item in cart" :key="item._id"> <!-- Loop through the cart items -->
        <!-- First Column: Image -->
        <div class="w-1/6">
        ...
        <div class="w-1/6 flex items-center">
          <button class="bg-orange-600 px-2" @click="updateQuantity(item._id, item.quantity - 1)">-</button> <!-- Decrease quantity -->
          <span class="mx-2">{{ item.quantity }}</span> <!-- Quantity -->
          <button class="bg-teal-600 px-2" @click="updateQuantity(item._id, item.quantity + 1)">+</button> <!-- Increase quantity -->
        </div>
      ...
      <div class="mt-4 pt-4">
        ...
        <div class="flex justify-end">
          <button class="bg-orange-600 text-white p-2 rounded hover:bg-orange-700" @click="placeOrder">Buy Now</button> <!-- Checkout button on click -->
        </div>
      </div>
    </div>
  </div>
...
const { cart, updateQuantity, cartTotal, cartTotalIndividualProduct, salesTax, grandTotal, code, placeOrder } = useCart();
</script>
